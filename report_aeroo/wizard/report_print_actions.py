...
class report_print_actions(models.TransientModel):
    ...
    state = fields.Selection([('draft','Draft'),('confirm','Confirm'),
        ('done','Done'),],'State', index=True, readonly=True, default='draft')
    print_ids = fields.Text()
    report_id = fields.Many2one('ir.actions.report.xml', 'Report')
    
    ### ends Fields

    @api.onchange('out_format')
    @api.multi
    def onchange_out_format(self):
        out_format = self.env['report.mimetypes'].browse(int(self.out_format))
        self.out_format_code = out_format.code
        return False
