...
from odoo import registry, osv,  models, _
...
class ExtraFunctions(object):
    ...
    def __init__(self, cr, uid, report_id, context, env):
        self.cr = cr
        self.uid = uid
        ...
        self.functions = {
            ...
            'currency_to_text': self._currency2text(context['company'].currency_id.name),
        # self._currency2text(context['company'].currency_id.code),
            ...
            'text_markdown': markdown_ok and self._text_markdown or \
                             self._text_plain(
                                 '"markdown" format is not supported! Need to be installed "python-markdown" package.'),
            'text_restruct': rest_ok and self._text_restruct or \
                             self._text_plain(
                                 '"reStructuredText" format is not supported! Need to be installed "python-docutils" package.'),
            'text_wiki': wikitext_ok and self._text_wiki or \
                         self._text_plain(
                             '"wikimarkup" format is not supported! Need to be installed "python-mediawiki" package.'),
    ...
    def _dec2time(self, dec, h_format, min_format):
        if dec == 0.0:
            ...
            return h_format.replace('%H', str(int(dec))) + min_format.replace('%M',
                                                                              str(int(round((dec - int(dec)) * 60))))
    ...
    def _currency2text(self, currency):
        def c_to_text(sum, currency=currency, language=None):
            ...
            return unicode(supported_language.get(language or self._get_lang()).currency_to_text(sum, currency),
                           "UTF-8")
    ...
    def _translate_text(self, source):
        ...
        trans = trans_obj.search([('res_id', '=', self.report_id), ('type', '=', 'report'), ('src', '=', source),
                                  ('lang', '=', self.context['lang'] or self.context['user_lang'])])
        # mod from https://github.com/aeroo/aeroo_reports/pull/57/files
        if trans:
            return trans_obj._get_source('ir.actions.report.xml', 'report', self._get_lang(),
                                         source=source, res_id=self.report_id)

        trans = trans_obj.search([('type', '=', 'report'), ('src', '=', source),
                                  ('lang', '=', self.context['lang'] or self.context['user_lang'])])
        if trans:
            return trans_obj._get_source('ir.actions.report.xml', 'report', self._get_lang(),
                                         source=source)

        trans_obj.create({'src': source, 'type': 'report', 'lang': self._get_lang(), 'res_id': self.report_id,
    ...
    def _get_label(self, obj, field):
        if not obj:
            return ''
        try:
            if isinstance(obj, osv.orm.browse_record):
                obj = obj[0]
    ...
    def _field_size(self, obj, field):
        try:
            ...
            if isinstance(obj, (str, unicode)):
                ...
                model = obj._name
            if isinstance(obj, (str, unicode)) or hasattr(obj, field):
                size = self.env[model].fields_get([field])[field]['size']
                return size
        except Exception, e:
            return ''
    ...
    def _field_accuracy(self, obj, field):
        try:
            ...
            if isinstance(obj, (str, unicode)):
                ...
                model = obj._name
            if isinstance(obj, (str, unicode)) or hasattr(obj, field):
                digits = self.env[model].fields_get([field])[field]['digits']
        ...
        def get_selection_item(obj, field, value=None):
            try:
                ...
                if kind == 'item':
                    if field_val:
                        return dict(
                                self.env[model].fields_get(allfields=[field])[field]['selection'])[field_val]
                elif kind == 'items':
    ...
    def _search(self, model, domain):
        obj = self.env[model].with_env({'lang': self._get_lang()})
        return obj.search(domain)
