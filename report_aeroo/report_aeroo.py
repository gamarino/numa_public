...
class Aeroo_report(report_sxw):
    ...
    def aeroo_docs_enabled(self, cr):
        ...
        '''
        pool = modules.registry.RegistryManager.get(cr.dbname)
        cr = pool.cursor()
    ...
    def __init__(self, cr, name, table, rml=False, parser=False, header=True, store=False):
        ...
        try:
            ...
            report_xml = ir_obj.search([('report_name', '=', name)], limit=1)
    ...
    def _subreport(self, cr, uid, aeroo_print, output='odt', aeroo_docs=False, context={}):
        ...
        def odt_subreport(name=None, obj=None):
            ...
            if report_xml_ids:
                service = odoo.report.interface.report_int._reports['report.%s' % name]
        ...
        def raw_subreport(name=None, obj=None):
            ...
            if report_xml_ids:
                ...
                report, _ = odoo.report.interface.report_int._reports['report.%s' % name].create_genshi_raw_report(cr, uid, \
    ...
    def create_genshi_raw_report(self, cr, uid, ids, data, report_xml, context=None, output='raw', tmpl=False):
        ...
        xfunc = ExtraFunctions(cr, uid, report_xml.id, oo_parser.localcontext,
                               odoo.api.Environment(cr, uid, oo_parser.localcontext or {}))
    ...
    def create_aeroo_report(self, cr, uid, ids, data, report_xml, context=None, output='odt'):
        ...
        user_name = pool.get('res.users').browse(uid).name
        model = pool.get('ir.model').search([('model','=',context.get('active_model', data['model']) or data['model'])])[0]
            ...
            if aeroo_docs and docs_client:
                try:
                    ...
                    if not report_xml.fallback_false:
                        ...
                        raise exceptions.UserError(_('Could not connect to Aeroo DOCS!'))
                    aeroo_print.subreports = []
            else:
                if report_xml.fallback_false:
                    #TODO should be removed or changed with DOCS
                    if not aeroo_docs:
                        raise exceptions.UserError(_('Aeroo DOCS connection is disabled.'))
                    elif not docs_client:
                        raise exceptions.UserError(_('Aeroo DOCS related error!'), _('Could not create Aeroo DOCS connection object.'))
                else:
