...
class report_xml(models.Model):
    ...
    def link_inherit_report(recs, new_replace_report_id=False):
        ...
        if new_replace_report_id:
            ...
            inherit_report = recs.replace_report_id
    ...
    def _report_content(recs, args=None):
        res = {}
    ...
    @api.multi
    def change_input_format(self, in_format=None):
        if not in_format:
            formats = self.env['report.mimetypes'].search([('code','=',self.in_format)])
        else:
            formats = self.env['report.mimetypes'].search([('code','=',in_format)])
        if self and formats:
            self.out_format = formats[0].id
    ...
    ### Fields
    charset = fields.Selection('_get_encodings', string='Charset', default='utf_8')
    ...
    report_sxw_content = fields.Binary('SXW content', compute='_report_content')
    active = fields.Boolean('Active', default=True, help='Disables the report if unchecked.')
    ...
    @api.multi
    def _set_auto_false(self):
        if not self:
            self = self.search([('report_type','=','aeroo'),('auto','=','True')])
        self.write({'auto':False})
        return True
